...
export interface components {
    schemas: {
        ...
        AttributionView: {
            ...
            /**
             * End
             * @description Last date a contribution was computed for, in the same full ISO-8601 form as `start`. Resolved: the latest date the run covers at or before the requested `end`, which `requested_end` echoes. `start` is the other end.
             */
            end: string;
            /** Index Id */
            index_id: string;
            ...
            /**
             * Requested End
             * @description The window's last date as asked for, YYYY-MM-DD, on the same terms as `requested_start`, which is the other end: the `end` query echoed unchanged, or the run's own last date when none was given. `end` is the resolved counterpart.
             */
        ...
        OverviewView: {
            ...
            /**
             * End
             * @description Last date the stored run's level series covers, YYYY-MM-DD, in the same form as `start`. Resolved from the data: earlier than the end the backtest requested whenever the price store stops short of it. `start` is the other end.
             */
            end: string;
            /** Index Id */
            index_id: string;
        ...
        ScheduleView: {
            ...
            /**
             * Recent
             * @description Rebalances already passed, most recent last. Trimmed to the `limit` most recent; `recent_total` says how many there were.
             */
            recent?: string[];
            /**
             * Recent Total
            ...
            /**
             * Upcoming
             * @description Scheduled rebalances after `as_of`, soonest first. Trimmed to the `limit` soonest; `upcoming_total` says how many were found.
             */
            upcoming?: string[];
            /**
             * Upcoming Total
    ...
    schedule_indices__index_id__schedule_get: {
        parameters: {
            query?: {
                ...
                asof?: string | null;
                /** @description Maximum dates per list, at most 512 and 4 by default. It bounds `recent` and `upcoming` separately rather than the two together, so a limit of 10 can return 20 dates. Raising it yields more history only: the lookahead is a separate bound, so it cannot produce further future dates. */
                limit?: number;
            };
            header?: never;
            path: {
